...
> 工作規格：[doc/design/specs/2026-06-18-motion-mode-design.md](specs/2026-06-18-motion-mode-design.md)

#### 現役實作：剛性節段旋轉（v1）

現役 3D 資產**無骨架（0 skins）**，無法使用骨架 rig 驅動。v1 採**剛性節段旋轉**：將人體拆分為節段樹，每個節段對應一組 Babylon `TransformNode`（樞紐），運動模式下以**逐自由度（per-DOF）滑桿**驅動旋轉，運動學邏輯位於 `app/utils/humanModel/motion/`。

**互動流程**：

1. 使用者開啟**運動模式（motion mode）**開關，顯示器進入可動狀態。
2. 各關節依其 `degreesOfFreedom` 展開對應滑桿（屈／伸、外展／內收、旋轉等）。
3. 拖曳滑桿→ `clampAngle` 將角度鉗制於 ROM `min`／`max` 內；達到極限時以**文字說明＋琥珀色提示**告知（§3.6，非僅顏色）。

**v1 覆蓋範圍（SFMA 臨床優先 6 關節）**：

| 關節 | 單樞紐近似 |
| --- | --- |
| 頸椎（cervicalSpine） | 是（多椎以單樞紐近似） |
| 肩（shoulder） | 否 |
| 脊椎（thoracicLumbarSpine） | 是（多椎以單樞紐近似） |
| 髖（hip） | 否 |
| 膝（knee） | 否 |
| 踝（ankle） | 否 |
...
> **待**（以下為後續軌，v1 不含）：
> - **on-model 拖曳手柄**：模型表面直接拖曳驅動關節（取代滑桿）
...
> - **軸／sign 實機目視校正**：各 DOF 旋轉軸方向與正負號待真實資產上目視驗證

### 4.3.4 肌肉收縮／伸展色彩

- 關節活動時，相關肌群依其狀態以**顏色顯示收縮與伸展**。
- 建議色彩映射：發散色階，收縮趨向暖色（紅）、伸展趨向冷色（藍），中性為基準色；強度對應長度變化量。
- 實作：由骨架關節角度推算各肌肉長度變化，驅動 **Babylon Node Material** 的材質參數（取代 Unity Shader Graph）。
...
工時集中於步驟 3（骨架對位）與步驟 4（跨關節肌權重清理）；其餘可腳本化重複執行（含正式資產替換佔位後重跑）。**減面品質**為主要風險：Z-Anatomy 為研究級高面數，可能需重拓樸才能在預算內保持乾淨且可蒙皮。**跨關節肌（軟蒙皮）清單**屬管線編寫資料、不入 runtime 實體；初版保守、視 QA 擴充。
