...
- 模型以**骨架（skeleton／bones）**驅動，關節可活動；骨架來源為 MakeHuman（≤120 bones，見 §4.6）。
- **綁定策略＝混合**：預設逐 mesh 剛性綁主骨；**跨關節肌**做 skin weights 軟變形（清單屬管線編寫資料，見 §4.6.3）。
- **不可超出人體活動範圍**：每個關節定義各自由度的角度上下限（ROM），拖曳到邊界即停止。
- 實作：Babylon **`BoneIKController`（反向運動學）＋ 自訂關節角度限制**，於更新迴圈鉗制（clamp）各關節角度於 ROM 內。
- 達到 ROM 極限時給予視覺提示（例如關節控制點轉為警示色），並同步以文字／圖示提示，不僅依賴顏色。

ROM 以資料定義，每個關節一筆（範例見 [06_data_model.md](06_data_model.md) 的 `joint`），便於依文獻調整與本地化。

> **待**：ROM clamp 之實際鉗制待 MakeHuman 骨架到位（目前種子 ROM 為教科書佔位值，發布前須 PT 審閱）。
