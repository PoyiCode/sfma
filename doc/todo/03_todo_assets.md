...
- [ ] **（管線實作分階段）** A／B／C 三階段已落地（技術＝幾何輪廓投影、非 Freestyle；`export2dSvg.py` 與 3D 共用 `pipelineCommon.py` 解析碼、`manifestV1.json` 單一真相、color-agnostic 由 app 套 CSS）；**唯像素級視覺精緻度待使用者實機 QA 迭代**（04 §4.6.3 步驟 7、[plans/2026-06-16-2d-stage-b-silhouette.md](../plans/2026-06-16-2d-stage-b-silhouette.md)）
