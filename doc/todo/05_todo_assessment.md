...
- [x] **動作參考圖**：判讀卡（`AssessmentEntryCard`）於判讀方格旁顯示該動作參考圖，供 PT 對照動作。圖源為官方 SFMA score sheet（`doc/ref/SFMA_form.pdf` 第 33 頁）裁切，存 `app/assets/sfma/patterns/<patternKey>.png`，經 `patternImage.ts`（`import.meta.glob`）匯入（base 安全、precache 可離線）（見 05 §5.2）。**註：圖源同受 SFMA 授權約束，見下「待決事項」。**
