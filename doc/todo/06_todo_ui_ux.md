...
- [ ] 人體模型檢視畫面：控制項清單依 03 §3.5（分層、單一部位還原、標籤、LOD、視角），與 [04](04_todo_human_model.md) 整合。**已完成**：各斷點／方向欄佈局——純函式 `render/modelPageLayout`（Expanded 任一方向／Medium 橫式→模型檢視器與身體標註清單並排 `split`；手機任一／平板直式→堆疊 `stack`），`.modelViewerPage[data-page-layout]` CSS 切換、單元測試。**仍待（待實機）**：精準貼近部位定位（待真實資產執行期螢幕座標）；3D 執行期 FPS 動態降級（已接線、待實機目視）
